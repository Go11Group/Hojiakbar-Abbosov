...
func GetCoursesByUUIDUser(c *gin.Context) {
    ...
    rows, err := database.DB.Query(`SELECT c.course_id, c.title, c.description FROM enrollments e JOIN courses c ON e.course_id = c.course_id WHERE e.user_uuid = $1 AND c.deleted_at IS NULL`, userUUID)
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
        return
    }
    ...
    c.JSON(http.StatusOK, gin.H{"user_uuid": userUUID, "courses": courses})
}
...
func SearchUsers(c *gin.Context) {
    ...
    if name != "" {
        queryBuilder.WriteString(" AND name ILIKE $" + string(argCounter))
        args = append(args, "%"+name+"%")
        argCounter++
    }

    if email != "" {
        queryBuilder.WriteString(" AND email ILIKE $" + string(argCounter))
