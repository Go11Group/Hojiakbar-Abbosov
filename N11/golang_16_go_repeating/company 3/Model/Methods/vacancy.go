...
func (recuiter *Recruiter) CreateVacansy(recuredId, companyId int, vakansy1 Vacancy)  {
	recuiters := []Recruiter{}
...
func (recuiter *Recruiter) DeleteVacansy(recuiredId, companyid1, vacansyId int)  {
	recuiters := []Recruiter{}
...
func (recuiter *Recruiter) UpdateVancy(recuireId3, companyid3, vacancyId3 int, vacansy1 Vacancy)  {
	reuciters := []Recruiter{}
		...
		if recuiters[i].Id == recuiterId4 {
			for j := 0; j < len(recuiters[i].Companies); j++ {
				if recuiters[i].Companies[j].Id == companyId4 {
					for k := 0; k < len(recuiters[i].Companies[j].Vacancies); k++ {
						fmt.Println("Vakansiya Soni :",recuiters[i].Companies[j].Vacancies[k].Count,"Vakansiya Turi: ", recuiters[i].Companies[j].Vacancies[k].Comments)
					}
				}
			}
		}
